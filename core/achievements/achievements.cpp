...
// Only include RAIntegration headers if supported (Windows only via CMake)
...
// Use nlohmann/json instead of rapidjson to avoid extra dependencies
#include <json.hpp>

#ifdef _WIN32
#include <windows.h>
// RA_Defs.h contains Windows-specific types (HWND, LPARAM) so it must be included after windows.h
#include "RA_Defs.h"
#endif
...
#ifdef _WIN32
	// Global window hooks for subclassing (Windows specific for RAIntegration DLL)
...
#define WM_RA_INVOKE_DIALOG   (WM_USER + 235)
	...
	{
#if defined(__ANDROID__)
		// On Android, delegate to Java SoundPool
		android_play_sound(filename.c_str());
#elif defined(USE_SDL)
		if (audioDevice == 0) return;

		// Perform file I/O and conversion on the worker thread to prevent blocking
		asyncTask([this, filename]() {
			...
			// Build converter from WAV format to opened Audio Device format
			SDL_AudioCVT cvt;
	...
	void Achievements::parseRarityJson(const char *json_data, size_t length)
	{
		...
		// Check if there is a top-level "Success" field
		...
		if (doc.contains("Sets") && doc["Sets"].is_array()) {
			// Format: getgameextended (used by official API)
			...
			// Format: patch/{id}.json (used by rcheevos internal) or getgame
	...
	void Achievements::fetchRarityData(u32 gameId)
	{
		asyncTask([this, gameId]() {
			// 1. Try to load from local RACache file (RA Integration stores it here)
			...
			// 2. Fallback to Network Fetch if cache not found
	...
	void Achievements::setManualIntegrationEnable(bool enable) {
		...
		cfgSaveBool("config", "EnableRAIntegration", enable);
	...
	void Achievements::runFrame() {
		// Sync Hardcore mode state from rc_client (RAIntegration DLL) to Flycast settings
		if (rc_client) {
			bool clientHardcore = (bool)rc_client_get_hardcore_enabled(rc_client);
			// Only update if logged in to avoid overwriting user config during init
			if (isLoggedOn() && clientHardcore != settings.raHardcoreMode) {
				settings.raHardcoreMode = clientHardcore;
				config::AchievementsHardcoreMode = clientHardcore; // Sync back to config too
				INFO_LOG(COMMON, "RA: Synced Hardcore Mode state from client: %s", clientHardcore ? "ON" : "OFF");
			}
		}

		// Handle pending resets on the UI thread to avoid crashes/race conditions
		if (resetPending) {
			resetPending = false;
			INFO_LOG(COMMON, "RA: Performing scheduled emulator reset (Reloading game)");

			// Use gui_start_game to perform a full clean reload of the content.
	...
	void Achievements::raintegrationCallback(int result, const char *error_message, rc_client_t *client, void *userdata)
	{
		...
		if (result == RC_OK) {
			...
			os_notify("RA Integration Loaded", 2000);
		}
		else {
	...
	bool Achievements::init()
	{
		...
		{
#if defined(USE_SDL)
			// Initialize SDL Audio Subsystem for playing RA sounds
			if (SDL_InitSubSystem(SDL_INIT_AUDIO) < 0) {
				...
				// Open the device. allow_changes = 0 means SDL will convert for us if hardware doesn't support the requested format
				...
				if (audioDevice == 0) {
					...
					// Unpause immediately. We will just queue data when needed.
			...
			manualIntegrationEnabled = cfgLoadBool("config", "EnableRAIntegration", false);
	...
	void Achievements::authenticationSuccess(const rc_client_user_t *user)
	{
		...
		// Signal to Main Thread to rebuild menu (Enable logout/login items)
		if (raIntegrationLoaded) {
	...
	{
		// Sync the hardcore setting from the client as it might have been toggled in the menu
		...
		SaveSettings(); // Persist changes so they survive the reload in gui_start_game

		INFO_LOG(COMMON, "RA: Resetting emulator due to reset event (scheduled)");
		// Set flag to perform reset on the main UI thread to prevent race conditions/crashes
		resetPending = true;
	}
	...
	std::string Achievements::getGameHash()
	{
		...
		rc_hash_generate_from_file(hash, settings.platform.isConsole() ? RC_CONSOLE_DREAMCAST : RC_CONSOLE_ARCADE,
			settings.content.fileName.c_str());
	...
	void Achievements::gameLoaded(int result, const char *errorMessage)
	{
		...
		// Fetch rarity data for the current game
		fetchRarityData(info->id);

		// Play info sound when game data is loaded
		playSound("info.wav");
