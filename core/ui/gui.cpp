...
void gui_initFonts()
{
	...
	// We must reset MergeMode because we are starting a new font (largeFont)
	...
	if (data != nullptr) {
		font_cfg.MergeMode = true;
		io.Fonts->AddFontFromMemoryTTF(data.release(), dataSize, largeFontSize, &font_cfg, faRanges);
	}
...
void gui_open_settings()
{
	...
	if (gui_state == GuiState::Closed && !settings.naomi.slave)
	{
		...
		// This applies to opening the menu via TAB, Escape (if configured), or other hotkeys that trigger this function.
		if (settings.raHardcoreMode) {
			u64 now = getTimeMs();
			if (now - emu.getLastPauseTime() < 3000) {
				os_notify("Please wait 3 seconds before opening the menu.", 2000);
				return;
			}
		}
...
static void gui_display_commands()
{
	...
	ImGui::Begin("##commands", NULL, ImGuiWindowFlags_NoDecoration);
	{
		...
		// Reset Button
		if (ImGui::Button(ICON_FA_ROTATE_RIGHT "  Reset", ScaledVec2(buttonWidth, 50)))
		{
...
void os_notify(const std::string& message, int duration)
{
	osdOverlay.set(message, duration);
}
...
static std::string getFPSNotification()
{
	if (config::ShowFPS)
	{
		...
		if (fps >= 0.f && fps < 9999.f) {
			...
			snprintf(text, sizeof(text), "F:%4.1f", fps);
