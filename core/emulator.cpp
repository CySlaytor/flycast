...
void Emulator::start()
{
	...
	if (state != Loaded && state != Running) {
		WARN_LOG(COMMON, "Unexpected emu state %d", state.load());
		return;
	}
	state = Running;
	SetMemoryHandlers();
...
void Emulator::performSaveState(int slot)
{
	...
	if (settings.raHardcoreMode) {
		os_notify("Save States disabled in Hardcore Mode", 2000);
		return;
	}
	...
	try {
		...
		char msg[32];
		snprintf(msg, sizeof(msg), "State Saved - Slot %d", slot + 1);
		os_notify(msg, 2000);
	}
	catch (...) {
		os_notify("Error saving state", 2000);
	}
...
void Emulator::performLoadState(int slot)
{
	...
	try {
		...
		char msg[32];
		snprintf(msg, sizeof(msg), "State Loaded - Slot %d", slot + 1);
		os_notify(msg, 2000);
	}
	catch (...) {
		os_notify("Error loading state", 2000);
	}
}
...
void Emulator::checkHotkeys()
{
			...
			if (settings.raHardcoreMode) {
				if (state == Running) {
					...
					if (now - lastPauseToggleTime < 3000) {
						allowed = false;
						os_notify("Please wait 3 seconds before pausing again.", 2000);
					}
				}
	...
	if (check_action(EMU_BTN_FRAME_ADV)) {
		if (!(pressed_keys & EMU_BTN_FRAME_ADV)) {
			if (settings.raHardcoreMode) {
				os_notify("Frame Advance disabled in Hardcore Mode", 2000);
			}
			else {
...
bool Emulator::render()
{
	...
	if (state == Paused) {
		...
		// Force a present to ensure the OSD is visible and swapbuffers happens
		rend_present();
